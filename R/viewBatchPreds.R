...
viewBatchPreds <- function(dataLoader,
  ...
  if(probs == TRUE & mode=="multiclass"){
    ...
    thePredsGrid <- torchvision::vision_make_grid(preds, num_rows=nCols)$permute(c(2,3,1))$to(device="cpu")
    ...
    predsC3 <- predsC2$to(dtype=torch::torch_float32())
    ...
    usedR <- terra::unique(msk1)  |> as.vector() |> unlist()
    usedP <- terra::unique(pred1) |> as.vector() |> unlist()
    ...
    terra::plotRGB(img1, r=r, g=g, b=b, scale=255, axes=FALSE, stretch="lin")
    terra::plot(msk1,  type="classes", axes=FALSE, levels=catDataR$cNames, col=catDataR$cColors, main="Reference")
    terra::plot(pred1, type="classes", axes=FALSE, levels=catDataP$cNames, col=catDataP$cColors, main="Predictions")

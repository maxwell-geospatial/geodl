...
#' or predictor variabels as  each mini-batch size, number of channels, height pixel count, and width pixel count (imageShape); the mask shape
#' (maskShape); image band means (bndMns); image band standard deviations (bndSDs); \
#' and count of pixels in each class in the batch (mskCnts).
#'
...
describeBatch <- function(dataLoader, usedDS=FALSE){
  ...
  freqTble <- torch::torch_bincount(torch::torch_tensor(batch1$mask+1, dtype=torch::torch_long())$flatten())$cpu() |>
    torch::as_array() |>
    as.vector()

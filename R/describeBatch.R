...
describeBatch <- function(dataLoader, zeroStart=FALSE){
  ...
  maskFreq <- if(zeroStart) mask + 1L else mask

  freqTble <- torch::torch_bincount(maskFreq$to(dtype=torch::torch_long())$flatten())$cpu() |>
    as.array() |>
    as.vector()

  maxIndex <- torch::torch_max(maskFreq)$cpu() |> as.array() |> as.vector()
  minIndex <- torch::torch_min(maskFreq)$cpu() |> as.array() |> as.vector()

  bndMns <- torch::torch_mean(batch1$image, dim=c(1,3,4))$cpu() |> as.array() |> as.vector()
  bndSDs <- torch::torch_std(batch1$image,  dim=c(1,3,4))$cpu() |> as.array() |> as.vector()
